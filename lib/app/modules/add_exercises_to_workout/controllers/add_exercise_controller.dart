...
class AddExerciseController extends GetxController{
 AddExerciseController({required this.addExerciseUseCase, required this.getExercisesTypesUseCase});
 var daysOfWeek = <Rx<DayOfWeekModel>>[DayOfWeekModel(name: "Segunda", key: "Monday",  isActive: false).obs, DayOfWeekModel(name: "Terça", isActive: false).obs, DayOfWeekModel(name: "Quarta", isActive: false).obs, DayOfWeekModel(name: "Quinta", isActive: false).obs, DayOfWeekModel(name: "Sexta", isActive: false).obs, DayOfWeekModel(name: "Sabado", isActive: false).obs, DayOfWeekModel(name: "Domingo", isActive: false).obs].obs;
 ...
 addExercise(int workoutID, int type) async {
   List<String>? daysOfWeekSelected = daysOfWeek.where((p0) => p0.value.isActive == true).toList().map((element) => element.value.key!).toList();
   ExerciseEntity exercise = ExerciseEntity(exerciseType: exerciseTypeSelected.value, exerciseVariation: exerciseVariationTextController.text, repeatTimeInSeconds: int.parse(repeatTimeTextController.text), seriesNumber: int.parse(seriesNumberTextController.text), daysOfWeek: daysOfWeekSelected);
  ...
  _manageIndividualWorkoutController.getExercisesByDayOfWeek(workoutID, dayOfWeekSelectedInTab!);
 }
